...
static void and_(bool canAssign) {
  ...
  // it's okay to pop the Value on the stack, because the previosu
  // was was true if OP_POP executes. Therefore, the Value of the
  // and expression will be the right expression.
  emitByte(OP_POP);
...
static void or_(bool canAssign) {
  ...
  // if left hand expression true, jump to end,
  // because the `or` expression will be true.
  int endJump = emitJump(OP_JUMP);

  // jump over `OP_JUMP` (jump to end) left is false
  patchJump(elseJump);

  // pop left. value of `or` now only depends on right side.
  emitByte(OP_POP);
...
ParseRule rules[] = {
  ...
  [TOKEN_DOT]           = {NULL,     dot,    PREC_ASSIGNMENT},
